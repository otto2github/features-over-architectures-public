...
class Context:
    ...
    def finish(self):
        ...
        self.export_json('RUN_STATUS.json',{'version':self.p['tool_version'],'baseline':'frozen v16 inputs; fixed-prediction subset analysis','role':self.role,'state':state,'started':self.start,'finished':datetime.now().astimezone().isoformat(),'training_executed':False,'paper_files_modified':False,'network_requests':False,'source_writes_requested':False,'checks':self.status})
        ...
        lines=['# PeerJ 141707 fixed-prediction diagnostic results',f'设备：{self.role}；状态：{state}','', '本包仅含聚合结果和核验状态；不含逐公司预测、个人节点、案件号或底层表。','本次没有训练模型、改标签文件、改论文或上传网络。','缺失/歧义使用 BLOCKED 或 PARTIAL，不能当作零或已排除问题。','PASS/COMPLETE 仅表示对应程序检查执行完毕，不表示没有时点问题或已达到发表标准。','', '| 模块 | 状态 | 说明 |','|---|---|---|']
        for s in self.status:lines.append('| '+s['module']+' | '+s['state']+' | '+str(s.get('reason',s.get('note','')))+' |')
        p=self.summary/'summary.md';p.write_text('\n'.join(lines)+'\n',encoding='utf-8');self.exports.append(p)
