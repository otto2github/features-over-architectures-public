...
def encode_audittyp(s: str) -> int:
    ...
    if pd.isna(s) or not isinstance(s, str) or s.strip() == "":
        return 9
    text = str(s).strip()
    # 注意优先级: 先匹配修饰版 (带强调/带解释), 再匹配基础版
    for code in [2, 3, 4, 5, 6, 8]:
        for kw in AUDIT_TYPE_MAPPING[code]:
            if kw in text:
                return code
    # 最后判断标准无保留 (避免被"无保留"无差别匹配)
    if "标准" in text and "无保留" in text:
        return 1
    if "标准无保留" in text:
        return 1
    if "无保留" in text and not any(k in text for k in ["保留意见", "带强调", "带解释"]):
        return 1
    return 8  # 兜底: 其他非标
