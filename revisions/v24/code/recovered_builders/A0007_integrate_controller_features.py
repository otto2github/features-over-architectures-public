...
# 国资关键字 (实控人姓名 S0701b 文本)
# [BUG FIX 2026-05-08] 去掉过于宽泛的 "中央" / "政府" / "事业单位" / "国有"
#   保留 4 个高精度关键字 (S0701b 实测命名规范覆盖度高):
#   - 国资委  (含 国务院国资委 / 省/市/县 国资委 / 国资委直管央企)
#   - 国务院  (国务院国资委)
#   - 国有资产 (国有资产监督管理委员会)
#   - 人民政府 (XX 市人民政府 / XX 省人民政府)
#   - 财政部  (财政部直接持股)
...
def build_controller_features(
    ...
    # ctrl_state_owned_flag: 改用 S0701b (实控人名称) 文本匹配国资关键字
    # [BUG FIX 2026-05-08] 原版用 S0702b 但 CSMAR S0702b 是数字编码 (1100/3110/3200 等),
    #   中文关键字匹配 100% 不命中 → flag 全 0. 改用 S0701b 实控人姓名文本字段:
    #   样本: "国务院国有资产监督管理委员会" / "深圳市人民政府国有资产监督管理委员会"
    if "S0701b" in fy.columns:
        s0701b_str = fy["S0701b"].astype(str).fillna("")
        state_pat = "|".join(STATE_OWNED_KEYWORDS)
        fy["ctrl_state_owned_flag"] = s0701b_str.str.contains(
            state_pat, regex=True, na=False
        ).astype("int8")
    else:
        log.warning("HLD_Contrshr 缺 S0701b 字段, ctrl_state_owned_flag 全置 0")
