#!/usr/bin/env python3
"""
PeerJ 141707 — Label v1 Stage B3c
Corrected CNINFO annual-report public-anchor crawl.

B3b stopped because its probe judged route purity using A-share prefixes only.
CNINFO main-board routes can also return B-share codes (Shanghai 900xxx,
Shenzhen 200xxx). The fixed PeerJ panel itself contains only:
  - Shanghai main A: 60xxxx
  - STAR: 68xxxx
  - Shenzhen main A: 00xxxx
  - ChiNext: 30xxxx
so the crawl safely filters every announcement back to the pinned 51,675
firm-year panel.

Reads local panel; fetches CNINFO announcement metadata only.
No PDF download, no training, no final fraud labels.
Writes only under ~/peerj_141707_audit/.
--reveal selects the final report in Finder.
"""
...
OUT_ROOT = BASE / "label_v1_stageB3c"
...
ROUTES = (
    {"name":"sse_main", "column":"sse",  "plate":"sh",
     "panel_re":r"^60", "exchange_re":r"^(60|68|90)"},
    {"name":"sse_star", "column":"sse",  "plate":"shkcp",
     "panel_re":r"^68", "exchange_re":r"^(60|68|90)"},
    {"name":"szse_main","column":"szse", "plate":"sz",
     "panel_re":r"^00", "exchange_re":r"^(00|20|30)"},
    {"name":"szse_gem", "column":"szse", "plate":"szcy",
     "panel_re":r"^30", "exchange_re":r"^(00|20|30)"},
)
...
def load_panel():
    ...
    if len(keys)!=51675: raise ValueError(f"PANEL_COUNT_UNEXPECTED:{len(keys)}")
    return keys
...
def request_json(data, timeout=30, retries=4):
    ...
    for i in range(retries):
        try:
            ...
            last=e
            if i+1<retries: time.sleep(min(12,1.5*2**i)+random.uniform(.1,.5))
    raise RuntimeError(f"CNINFO_REQUEST_FAILED:{type(last).__name__}:{last}")

def cpath(route,fy,page):
    d=CACHE_ROOT/route["name"]/str(fy); d.mkdir(parents=True,exist_ok=True,mode=0o700)
...
def fetch(route,fy,page,delay):
    ...
    if not isinstance(obj,dict) or not isinstance(obj.get("announcements"),list):
        raise RuntimeError(f"BAD_RESPONSE_SHAPE:{route['name']}:{fy}:{page}")
...
def probe(route,delay):
    obj,src=fetch(route,2023,1,delay)
...
def main():
    ...
    ap.add_argument("--stage-b3c",action="store_true")
    ap.add_argument("--delay",type=float,default=1.2)
    ap.add_argument("--reveal",action="store_true")
    a=ap.parse_args()
    if not a.stage_b3c:ap.error("--stage-b3c required")
    if a.delay<.8:ap.error("--delay must be >= 0.8")

    os.umask(0o077)
    OUT_ROOT.mkdir(parents=True,exist_ok=True,mode=0o700)
    CACHE_ROOT.mkdir(parents=True,exist_ok=True,mode=0o700)
    out=OUT_ROOT/("stageB3c_"+datetime.now().strftime("%Y%m%d_%H%M%S")+"_"+uuid.uuid4().hex[:8]);out.mkdir(mode=0o700)
    report=out/"label_v1_stageB3c_report.txt"
    ...
    try:
        ...
        for code,fy in panel:
            for r in ROUTES:
                if re.search(r["panel_re"],code):
                    panel_routes[r["name"]]+=1; break

        print("[2/6] 探测CNINFO路由；允许主板接口中的B股，但面板只保留A股……",flush=True)
        probes={}
        for r in ROUTES:
            probes[r["name"]]=probe(r,a.delay)
            p=probes[r["name"]]
            print(f"      {r['name']}: ok={p['ok']} pages={p['pages']} exchange={p['exchange_purity']:.1%} panel-prefix={p['panel_A_prefix_share']:.1%} prefixes={p['sample_prefix2']}",flush=True)
        if not all(x["ok"] for x in probes.values()):
            lines=[
                "PeerJ 141707 | Label v1 Stage B3c route probe",
                ...
                "STATUS: LABEL_V1_STAGE_B3C_STOP_ROUTE_PROBE_FAILED",
        ...
        for fy in FISCAL_YEARS:
            for r in ROUTES:
                ...
                npage=pages(obj)
                if npage<=0 or npage>1000:raise RuntimeError(f"BAD_PAGE_COUNT:{r['name']}:{fy}:{npage}")
                rr=kk=0
                for pg in range(1,npage+1):
                    cur=obj if pg==1 else fetch(r,fy,pg,a.delay)[0]
                    if pg>1:req["network" if not cpath(r,fy,pg).exists() else "cache"]+=0
        ...
        for code,fy in sorted(panel,key=lambda z:(z[1],z[0])):
            ...
            rows.append({
                ...
                "source":"CNINFO announcement metadata; exchange+plate routed; panel-filtered",
            })
        write_parquet(out/"annual_report_public_anchors_CNINFO_v3.parquet",rows)

        with (out/"anchor_anomalies_STAGEB3C_LOCAL_ONLY.csv").open("w",encoding="utf-8",newline="") as f:
        ...
        lines=[
            "PeerJ 141707 | Label v1 Stage B3c corrected CNINFO public annual-report anchor",
            "NO FINAL LABELS; NO TRAINING; NO PDF DOWNLOAD",
            "Local time: "+datetime.now().astimezone().isoformat(),
            "Output: "+str(out),"",
            "A. PANEL ROUTE COUNTS",
            ...
            "E. YEAR-BY-YEAR COVERAGE",*yearlines,"",
            "F. DECISION",
        ]
        if cov>=.95 and normal/max(1,n)>=.99:
            lines += [
                ...
                "STATUS: LABEL_V1_STAGE_B3C_PUBLIC_ANCHOR_READY_FOR_FREEZE",
            ]
        else:
            lines += [
                "public_anchor_source_eligible_for_freeze=false",
                "Coverage/timing gate not met; residual missingness needs one final targeted diagnosis.",
                "STATUS: LABEL_V1_STAGE_B3C_STOP_COVERAGE_OR_TIMING_GATE",
            ]
        lines += ["","source_files_modified=false","training_or_prediction_run=false","final_nonnull_labels_written=0","Upload only label_v1_stageB3c_report.txt."]
        report.write_text("\n".join(lines)+"\n",encoding="utf-8")

        (out/"manifest_v1_stageB3c.json").write_text(json.dumps({
            "created_at":datetime.now().astimezone().isoformat(),
            "panel_sha256":PANEL_SHA256,"coverage":cov,
            "anchor_sha256":sha256(out/"annual_report_public_anchors_CNINFO_v3.parquet"),
        ...
        report.write_text(
            "PeerJ 141707 | Stage B3c FAILED SAFELY\n"
            f"ERROR_TYPE={type(e).__name__}\nERROR={e}\n"
            "source_files_modified=false\ntraining_or_prediction_run=false\nfinal_nonnull_labels_written=0\n"
            "STATUS: LABEL_V1_STAGE_B3C_FAILED_SAFE\n",encoding="utf-8")
        if a.reveal:reveal(report)
        raise
